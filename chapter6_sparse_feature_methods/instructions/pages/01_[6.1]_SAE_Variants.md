...
<details>
<summary>Interpretation - why retain the latent code?</summary>

The noiseless identity proves that the data generator is correct before optimization begins. Retaining `feature_acts` and the shared dictionary gives later recovery and AUC measurements an external ground truth instead of defining success from the SAE's own reconstruction.
...
<details>
<summary>Interpretation - what differs between the variants?</summary>

ReLU-L1 changes density through a loss penalty, TopK enforces an exact activation budget, gated separates feature selection from magnitude, and JumpReLU learns a discontinuous firing threshold. Matching their forward rules is necessary before comparing any training result.
...
<details>
<summary>Interpretation - why expose every loss term?</summary>

The four objectives place pressure on different quantities, so equal coefficients do not imply equal sparsity. Reporting reconstruction, raw sparsity, and the gated auxiliary term separately prevents a low total loss from hiding which tradeoff the optimizer actually made.
...
<details>
<summary>Interpretation - why are recovery and AUC separate?</summary>

Decoder recovery asks whether all planted directions were found; held-out AUC asks whether one train-selected latent predicts a known cause on unseen samples. Either can look strong while the other fails, and freezing selection prevents the held-out labels from choosing the answer.
...
<details>
<summary>Interpretation - what does the control isolate?</summary>

The matched and random interventions use the same strength or coefficients, so their difference is attributable to direction alignment. This establishes a causal effect on the planted projection only; it does not by itself establish a language-model behavior or a human-interpretable concept.
