...
> **Local-first extension.** Original ARENA material is unchanged. This extension section is a CPU-runnable planted crosscoder lesson; the optional paired TransformerLens CUDA preflight is owned by the parent GPU runner and is not the signature result.
...
These figures are generated by the solved notebook from a real CPU training run, not loaded as the main lesson output. They are included here so reviewers can see the expected shape of the result before opening the notebook.

<img src="../assets/crosscoder_training_reconstruction_curves.png" width="820">

<img src="../assets/crosscoder_latent_ownership_heatmap.png" width="820">

<img src="../assets/crosscoder_behavior_auc_baselines.png" width="820">

<img src="../assets/crosscoder_intervention_bars.png" width="820">

## Parent GPU Hook

`solutions.run_gpu_test(max_vram_gb=24.0)` remains available for the parent GPU runner. It should be treated as bounded real paired-model context until a real-model trained crosscoder is added; this section's graded learner-facing claim is the planted sparse-crosscoder result above. The refreshed parent report is `verification_report.json` and remains supporting evidence only.
