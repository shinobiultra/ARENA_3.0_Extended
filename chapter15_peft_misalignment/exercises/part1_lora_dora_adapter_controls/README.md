...
- `15.1_LoRA_DoRA_and_Adapter_Controls_exercises.ipynb` is the learner-facing
  notebook with nine tested implementation steps, real adapter training,
  held-out/OOD examples, controls, play cells, and anomaly hunting.
...
- `../../instructions/assets/lora_dora_adapter_controls_signature_panel.png`
  shows the training curves, update spectrum, matched behavior, mechanism
  directions, controls, and acceptance checks generated by the notebook.
- `../../instructions/assets/lora_dora_adapter_controls_behavior_table.png`
  shows concrete OOD examples where the distractor baseline flips but the
  trained adapter follows the planted target coordinate.
