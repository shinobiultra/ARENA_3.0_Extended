...
EXPECTED_RUNTIME = "45-60 minutes for the CPU theorem; minutes on the local GPU preflight"
REQUIRES_GPU = True  # CPU theorem is runnable now; parent runs the bounded CUDA preflight later
```

## Core Question

When an attribution graph says that a path carries a behavior, does that path make a quantitative prediction that survives causal intervention and beats matched controls?
