...
def _template_centered_activation_table(
    *,
    ...
    for template in templates:
        template_activations = _final_token_hidden_states(
            model=model,
            tokenizer=tokenizer,
            prompts=tuple(template.format(label) for label in label_names),
            device=device,
        )
        activations.append(template_activations - template_activations.mean(dim=0, keepdim=True))
        labels.append(label_ids)
    return t.cat(activations, dim=0), t.cat(labels, dim=0)
...
def _template_centered_identity_geometry(
    *,
    model,
    tokenizer,
    label_names: tuple[str, ...],
    train_template: str,
    heldout_template: str,
    device: t.device,
    noise_seed: int,
    min_centered_accuracy: float,
) -> dict:
    labels = t.arange(len(label_names), dtype=t.long)
    train_prompts = tuple(train_template.format(label) for label in label_names)
    heldout_prompts = tuple(heldout_template.format(label) for label in label_names)
    train_activations = _final_token_hidden_states(
        model=model,
        tokenizer=tokenizer,
        prompts=train_prompts,
        device=device,
    )
    heldout_activations = _final_token_hidden_states(
        model=model,
        tokenizer=tokenizer,
    ...
    centered = template_center_activations(t.stack([train_activations, heldout_activations]))
    centered_train = centered[0]
    centered_heldout = centered[1]
...
def pythia_weekday_geometry_preflight(
    *,
    ...
    """Check weekday identity geometry in a pinned real LM hidden-state cache."""

    if not t.cuda.is_available():
        raise RuntimeError("11.1 Pythia weekday geometry preflight requires CUDA.")
    ...
    weekday_geometry = _template_centered_identity_geometry(
        model=model,
        tokenizer=tokenizer,
    ...
    month_geometry = _template_centered_identity_geometry(
        model=model,
        tokenizer=tokenizer,
        label_names=MONTHS,
        train_template=MONTH_TRAIN_TEMPLATE,
        heldout_template=MONTH_HELDOUT_TEMPLATE,
        device=device,
        noise_seed=1,
        min_centered_accuracy=min_centered_accuracy,
    )
    weekday_vis_train, weekday_vis_train_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=WEEKDAYS,
        templates=WEEKDAY_VIS_TRAIN_TEMPLATES,
        device=device,
    )
    weekday_vis_heldout, weekday_vis_heldout_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=WEEKDAYS,
        templates=WEEKDAY_VIS_HELDOUT_TEMPLATES,
        device=device,
    )
    weekday_random_token_train, weekday_random_token_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=WEEKDAY_RANDOM_TOKEN_CONTROL_LABELS,
        templates=WEEKDAY_VIS_TRAIN_TEMPLATES,
        device=device,
    )
    month_vis_train, month_vis_train_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=MONTHS,
        templates=MONTH_VIS_TRAIN_TEMPLATES,
        device=device,
    )
    month_vis_heldout, month_vis_heldout_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=MONTHS,
        templates=MONTH_VIS_HELDOUT_TEMPLATES,
        device=device,
    )
    month_random_token_train, month_random_token_labels = _template_centered_activation_table(
        model=model,
        tokenizer=tokenizer,
        label_names=MONTH_RANDOM_TOKEN_CONTROL_LABELS,
        templates=MONTH_VIS_TRAIN_TEMPLATES,
        device=device,
    )
    weekday_visualization = _umap_visualization_sweep(
        train_activations=weekday_vis_train,
        train_labels=weekday_vis_train_labels,
        heldout_activations=weekday_vis_heldout,
        heldout_labels=weekday_vis_heldout_labels,
        random_token_activations=weekday_random_token_train,
        random_token_labels=weekday_random_token_labels,
        task_name="weekday",
    )
    month_visualization = _umap_visualization_sweep(
        train_activations=month_vis_train,
        train_labels=month_vis_train_labels,
        heldout_activations=month_vis_heldout,
        heldout_labels=month_vis_heldout_labels,
        random_token_activations=month_random_token_train,
        random_token_labels=month_random_token_labels,
        task_name="month",
    )
    ...
    return {
        ...
        "pca_explained_variance_ratio": weekday_geometry["pca_explained_variance_ratio"],
