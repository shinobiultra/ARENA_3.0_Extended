...
By the end of this notebook, you will have shown that a concise feature verbalizer only deserves belief when it predicts held-out counterfactual activations, because a pinned local language model revises its hypothesis after blind counterexamples and the revised executable rule beats lookup, random-word, and inactive controls on 24 untouched examples.
...
EXPECTED_RUNTIME = "60-90 minutes including the pinned local-model CPU loop"
REQUIRES_GPU = False
```
...
- Run a pinned local language model through hypothesis selection, revision, and blind scoring without treating fluent wording as proof.
...
The deterministic lexicon is exact ground truth, but it is not the verbalizer. A pinned local `Qwen2.5-0.5B-Instruct` now chooses a readable hypothesis from training evidence, sees three coherent revision counterexamples, fixes its choice, and is graded on 24 untouched examples. No external API or report replay participates.

### Exercise - build an auditable local-model hypothesis prompt

Render labeled training examples and a small hypothesis menu. The first prompt must contain no revision or held-out rows; the revision prompt may contain only mined counterexamples.

<details>
<summary>Expected output</summary>

All training rows appear in the initial prompt, all 24 held-out rows remain absent, and revision without counterexamples raises an error.
...
Keep candidate rules executable and candidate descriptions readable. The local model selects language; your exact toy oracle supplies labels only for scoring.
...
This separates hypothesis generation from evaluation. A model cannot earn held-out accuracy by seeing the held-out strings in its prompt.
...
### Exercise - score candidate hypotheses with a pinned local language model

Use the local Qwen next-token distribution over single-letter candidate keys. Return raw logits and probabilities among candidates, then select the maximum-scoring hypothesis.

<details>
<summary>Expected output</summary>

The pinned model selects `C`: living animal + resting action + physical surface.
...
Use the chat template, request one next-token distribution, and index logits for candidate key token IDs. Do not call an external API or parse free-form prose.
...
The model supplies a visible, reproducible semantic preference. Exact rules remain independently executable, so fluent wording cannot substitute for predictive performance.
...
### Exercise - revise on counterexamples, then open the blind held-out split

Apply the initial rule to the revision split, mine its shared failures, ask the local model to choose between keeping and revising the rule, and evaluate both hypotheses on 24 untouched examples plus controls.

<details>
<summary>Expected output</summary>

Three decoy counterexamples change the local choice from `C` to `D`; held-out accuracy rises from `75%` to `100%` and beats always-inactive, lookup, and random-keyword controls.
...
Do not add held-out examples to either prompt. Print both candidate score tables and every held-out row so the result is auditable.
...
The revision earns belief because one semantic change repairs a coherent error class and generalizes to untouched decoys, motion examples, substring traps, and positives.
...
## Local-Model Signature Result

![Local feature-hypothesis revision](../assets/local_feature_hypothesis_revision.png)

The initial local choice is candidate `C` and scores `75%` on the blind split. Three non-living replica counterexamples lead the same model to candidate `D`; the revised executable rule scores `100%`, while always-inactive, train-example lookup, and random-keyword controls remain below it.
