...
expected_runtime: 90-120 minutes including the pinned CPU Pythia lab
requires_gpu: false
```
...
When a decoder says an activation means a token, what evidence makes that claim
stronger than a prompt prior or a plausible story?
...
## Real CPU Lab: Six-Layer Pythia

The exact tensor exercises now escalate to a pinned local `EleutherAI/pythia-70m-deduped` run on CPU. Forty-eight training prompts fit stage-wise affine lenses; 24 disjoint held-out prompts produce every visible rank and activation-insertion metric.
...
![Pythia lens and activation insertion signature](../assets/pythia_lens_patchscope_cpu_signature.png)

The ordinary median target rank falls from `18163` at the embedding to `1` at the final stage. The ridge lens improves several early and middle stages but honestly worsens at least one later stage. With one block remaining, correct-source insertion recovers `87.5%` of 24 source targets, versus `0%` for text-only, cyclic wrong-source, and matched-scale random controls. Mean target log-probability gain is `7.73` nats for the correct source.

The image summarizes the curves and controls; the notebook's two 24-row tables are the evidence to inspect when an aggregate hides an exception.

## Try It Yourself

Change the insertion layer from block 5 to an earlier block, predict the direction of the accuracy and log-probability changes, then rerun the notebook plot. Next, choose one row where the ridge lens worsens target rank and inspect the ordinary and calibrated top tokens instead of averaging the anomaly away.
