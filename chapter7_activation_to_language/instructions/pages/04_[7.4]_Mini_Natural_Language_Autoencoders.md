...
**By the end of this notebook, you will have shown that an exact toy NLA preserves planted semantics, then a learned six-phrase text bottleneck on actual Pythia residuals recovers held-out next-token behavior while no-text, wrong-phrase, and shuffled-label controls fail.**
...
## Real CPU Lab: Pythia Residual to Text to Residual

After the exact toy theorem, the notebook caches final hidden states from pinned `EleutherAI/pythia-70m-deduped`. A learned encoder maps 36 residuals to six natural-language phrase classes. The decoder receives actual Pythia token embeddings for those phrases and reconstructs a residual; 18 disjoint prompts test phrase recovery, residual error, and recovered next-token behavior.

### Exercise - cache actual Pythia residuals with precommitted token classes

Run pinned Pythia on six prompt families. Cache final normalized hidden states for 36 training prompts and 18 disjoint evaluation prompts, and verify every model argmax matches the class defined before fitting.

<details>
<summary>Expected output</summary>

Residual shapes are `[36, 512]` and `[18, 512]`; all 54 targets match one of `a`, `the`, comma, hyphen, period, or line break.
...
Use left padding and take the final sequence position. Fail loudly if the pinned model no longer produces a curated target; do not silently relabel examples.
...
The concepts are properties of an actual model state. The labels are precommitted from the model's behavior, not synthetic coordinates pasted onto random tensors.
...
### Exercise - learn residual-to-phrase encoding on the real cache

Fit a ridge classifier from normalized residuals to six readable phrase classes. Predict the 18 blind phrase labels without looking at their prompts or token IDs.

<details>
<summary>Expected output</summary>

Blind phrase accuracy is at least `75%`; a shuffled-label encoder is at most `25%`.
...
Solve ridge in the 36-example dual space. Keep normalization statistics from the training split and reuse them unchanged at evaluation.
...
The encoder asks whether phrase classes are available in residual geometry. Shuffled labels test whether the same capacity can manufacture the result.
...
### Exercise - decode actual phrase embeddings back into residual space

Mean Pythia's own token embeddings for each natural-language phrase, then learn a least-squares map from those text features to class-mean residuals. Never index a learned residual table by phrase ID.

<details>
<summary>Expected output</summary>

The phrase feature matrix is `[6, 512]`, has rank six, and oracle phrase reconstructions beat the no-text mean in held-out MSE.
...
Build text features by calling `model.get_input_embeddings()` on tokenized phrases. The phrase ID is used only to select which natural-language embedding enters the decoder.
...
The bottleneck is genuinely textual: changing the phrase changes its model embedding before reconstruction. The linear decoder still limits what claims we can make.
...
### Exercise - evaluate phrase, reconstruction, and behavior controls together

Run the learned encoder and text decoder end to end. Compare residual MSE and recovered next-token behavior against no text, wrong phrase, and shuffled encoder labels; print all 18 rows.

<details>
<summary>Expected output</summary>

Phrase and reconstructed-token accuracy are `77.8%`; oracle phrases recover `100%`; no text gets `16.7%`; wrong phrases get `0%`. MSE orders learned text < no text < wrong phrase.
...
Remember that cached states are already final-normalized, so unembed reconstructions directly. Applying final layer norm twice changes the question and breaks the target check.
...
The result separates two failures: four encoder errors and no decoder loss of target identity when the phrase is correct. Controls show that text identity, not merely a class-average residual, determines the recovered behavior.
...
## Real-Model Signature Result

![Pythia residual text bottleneck](../assets/pythia_real_text_bottleneck.png)

The learned phrase bottleneck reaches `77.8%` phrase and reconstructed-token accuracy. Correct oracle phrases recover `100%` of the six token classes, compared with `16.7%` for no text and `0%` for cyclic wrong phrases. Residual MSE orders learned text (`2.62`) below no text (`3.34`) below wrong phrase (`4.94`); shuffled encoder labels reach only `11.1%`.

<!-- REAL_CPU_LAB_END -->

## Verification Handoff

The visible signature result above is recomputed from the learner functions. The committed report records the separate pinned real-model preflight and is never substituted for the exact model-organism result.
...
def run_gpu_test(max_vram_gb: float = 24.0) -> dict:
    ...
    verification = import_module(
        "chapter7_activation_to_language.exercises.part4_mini_natural_language_autoencoders.solutions"
    )
