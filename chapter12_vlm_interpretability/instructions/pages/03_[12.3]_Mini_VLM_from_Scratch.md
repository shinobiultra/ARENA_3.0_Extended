...
**Common bug:** treating a successful object patch as localized evidence without checking the token geometry and same-size background controls. The causal claim only passes when the object rows flip the answer and matched non-object rows preserve it.
...
REQUIRES_GPU = True  # the learner result is CPU-friendly; the release contract includes CUDA verification
```
...
<details><summary>Help - check the reasoning before opening the solution</summary>Start with the exact additive token game. Verify the object-box-to-token mapping and signed answer margin there before interpreting the trained model.</details>

<details><summary>Interpreting the result</summary>The result is causal only because the object patch flips the answer while text-only, image-only, shuffled-visual, background, and same-size random-region controls fail.</details>
...
Every exercise has immediate semantic tests. The notebook also provides expected-output, help, interpretation, and full solution dropdowns beside each task.
